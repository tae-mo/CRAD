...
def main():
    ...
    if args.evaluate: config.eval_mode = True
    config.exp_path = os.path.dirname(config.saver.exp_path)
    ...
    config.feat_path = os.path.join(config.exp_path, config.saver.feat_dir)
    config.evaluator.vis_compound.save_dir = os.path.join(config.exp_path, config.evaluator.vis_compound.save_dir)
    ...
    if rank == 0:
        ...
        os.makedirs(config.feat_path, exist_ok=True)
    ...
    for name, p in model.named_parameters():
        if len(name.split('.')) > 2 and 'query' in name.split('.')[2]: 
            query.append(p)
            continue
        params.append(p)
        
    optimizer = torch.optim.AdamW([{'params': params, 'lr': config.trainer.net_lr},
                                   {'params': query, 'lr': config.trainer.grid_lr},])
    ...
    if args.evaluate:
        ...
        if config.generate_feature:
            if rank == 0: logger.info("Generaing Feature...")
            generate_feature(val_loader, model)
        return
...
def train_one_epoch(
    ...
    for i, input in enumerate(train_loader):
        ...
        dist.all_reduce(reduced_loss) # default op: ReduceOp.SUM
        reduced_loss = reduced_loss / world_size # div by world_size after gathering all values
...
def validate(val_loader, model):
    ...
    if outputs.get('time', None):
        for k, v in outputs['time'].items():
            ...
            logger.info(f"[{k}] mean elapsed time: {avg_elapsed:.4f} ms")
...
def generate_feature(val_loader, model):
    global idx
    
    model.eval()
    with torch.no_grad():
        for i, input in enumerate(val_loader):
            input['save_recon'] = True
            outputs = model(input)
